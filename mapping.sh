...
grep ${SAMPLE_NAME} 20130502.phase3.index | grep "exome" | grep "Pond-" | grep 'PAIRED' | grep -v 'Solexa' | grep -v 'from blood' | grep -v '_1.filt.fastq.gz' | grep -v '_2.filt.fastq.gz' | sed 's/\t/,/g' > father.index
